...
- `SCOPE.md` (on the `Codex/dreamy-robinson-9fcc52` branch in PR #162) — authoritative ServiceM8+GHL scope
...
### MCP server — drive the app from Codex / Codex.ai

A remote **Model Context Protocol** server at **`/api/mcp`** (`mcp-handler` package) lets Codex, the Codex API, and the **Codex.ai connector** operate a business end-to-end. ~50 tools across every entity.
...
### Client onboarding form builder (shipped — PRs #343/#344/#345, June 2026)
- **A per-business "plugin"** (user requirement: enable/disable per business). Flag = `onboarding_settings.enabled`; togglable from the **/agents store** ("Client Onboarding" card — `syncSideEffects` in `src/lib/actions/agents.ts` syncs it) OR the enable card at `/onboarding-forms`; `setOnboardingEnabled` mirrors back into `business_agent_installs`. Conditional sidebar item "Onboarding" under Contacts (feature-flag mechanism shared with quotingAgent — flags fetched in parallel in `(dashboard)/layout.tsx`).
- **⚠️ Routed at `/onboarding-forms`** — `/onboarding` is the signup wizard; don't collide.
- **Tables** (migration `20260624100000_client_onboarding.sql`, applied + recorded): `onboarding_settings` / `onboarding_forms` (schema JSONB = ordered `OnboardingField[]`) / `onboarding_requests` (pending/viewed/completed, portal-token delivery) / `onboarding_responses` (answers keyed by field id + **schema_snapshot** so form edits don't orphan answers, `draft` flag for autosave) + private `onboarding-uploads` bucket. Standard RLS.
- **25 field types** (`OnboardingFieldType` in database.ts) incl. image/file uploads, opening-hours grid, ABN, rating, consent, `show_if` conditional visibility, and **`secure`** credentials.
- **🔐 Secure fields**: AES-256-GCM via `src/lib/onboarding/crypto.ts`, key = `ONBOARDING_SECRET_KEY` env (64 hex chars — **still needs setting on Vercel**; feature cleanly unavailable without it). Ciphertext never reaches the browser or MCP; reveal = `revealSecureAnswer` (owner/admin only). Shared plain helpers in `src/lib/onboarding/answers.ts` (encrypt-for-storage / redact / required-validation — no "use server").
- **Surfaces:** `/onboarding-forms` (forms grid + Sent tab), `/onboarding-forms/[id]` (builder: palette → reorder canvas (framer-motion Reorder) → settings panel; Preview mode; `?response=<requestId>` renders the response viewer with signed-URL file open + secure Reveal). Customer fill at `/portal/[token]/onboarding/[requestId]` (autosave merge-patches to `…/save`, uploads to `…/upload` 10MB, progress bar, thank-you). Actions in `src/lib/actions/onboarding.ts`.
- **MCP** (scopes `onboarding:read/write`): `set_onboarding_enabled`, `list/get/create/update/delete_onboarding_form` (AI can draft a form from a description), `send_onboarding_form`, `list/delete_onboarding_request`, `get_onboarding_response` (always redacted).
- Icons gained `Lock` + `GripVertical` (DotsSixVertical) aliases.
- **View form**: form cards have a View button → `/onboarding-forms/[id]?view=1` opens the builder in Preview mode.

### Public Form Builder (shipped — PRs #351/#352/#353, June 2026)
- **Separate per-business "plugin"** from onboarding — public/embeddable **lead-capture** forms (Typeform/GHL style). Flag `form_builder_settings.enabled`; togglable via the **/agents store** ("Form Builder" card, category leads) or the enable card at `/forms`; `setFormBuilderEnabled` mirrors into `business_agent_installs`. Conditional "Forms" sidebar item under **Sales** (feature `formBuilder`).
- **Reuses the onboarding field engine** — `public_forms.schema` is `OnboardingField[]`; shares `presets.ts` + `answers.ts` validation. Public forms **exclude the `secure` field type**.
- **Tables** (migration `20260624140000_form_builder.sql`, applied + recorded): `public_forms` (name, **globally-unique `slug`**, status draft/published/archived, schema, `settings` {submit_text, thank_you_message, redirect_url, `create_lead` default on, `lead_map` {name,email,phone→field id}, notify_emails}, `theme` {accent, show_branding}) + `public_form_submissions` (answers, `lead_id`, meta ip/ua/referrer) + private `public-form-uploads` bucket. Standard RLS.
- **Delivery:** public hosted page `/f/[slug]` + iframe `/embed/[slug]` (shared `PublicFormShell`; no frame headers so it embeds anywhere). Embed snippet in `src/lib/forms/embed.ts`.
- **Public APIs (slug-gated, no auth, middleware-whitelisted `/f/ /embed/ /api/f/`):** `POST /api/f/[slug]/submit` (honeypot `_hp` + in-memory `rateLimit` 8/min/IP + server validation; stores submission; optional lead via `upsert_lead` source `form`; emails `notify_emails`; returns redirect_url), `/upload` (10MB), `/file?path=` (image preview scoped to the form's namespace).
- **Surfaces:** `/forms` (list + copy link/embed + create), `/forms/[id]` (builder Build/Settings/Submissions tabs — Settings has create-lead toggle + field mapping + notify emails + thank-you/redirect). Actions in `src/lib/actions/forms.ts`.
- **MCP** (scopes `forms:read/write`): `set_form_builder_enabled`, `list/get/create/update/delete_public_form`, `list_form_submissions` — list/get return `public_url` + `embed_url`.
