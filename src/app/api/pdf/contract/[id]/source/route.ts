...
/** Serves a contract's uploaded source PDF. Accepts a portal ?token= or an authed user. */
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  return new NextResponse(buf, {
    headers: { "Content-Type": "application/pdf", "Content-Disposition": `inline; filename="${(c.title || "contract").replace(/[^a-z0-9]+/gi, "-")}.pdf"` },
  });
}
