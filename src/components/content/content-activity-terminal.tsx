...
import { listContentActivity } from "@/lib/actions/content";
import { cn } from "@/lib/utils";

interface Event { id: string; agent_id: string | null; level: string; message: string; cost_cents: number; created_at: string }

/** Live terminal of agent activity for a brand — polls every 3s and auto-scrolls.
 *  Lifted from the SEO one: 56 lines, poll-based, no Realtime. It works. */
export function ContentActivityTerminal({ brandId }: { brandId: string }) {
  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        const e = await listContentActivity(brandId);
