...
### Customer contracts + e-signature (shipped — PR #336, branch `feat/contracts`, NOT yet merged)
- **Tables** (migration `20260623100000_contracts.sql`, applied + recorded on remote): `contract_templates` (reusable bodies) + `contracts` (id, business_id, user_id, customer_id, title, `kind` `rich_text`|`pdf`, content_html, source_path, `status` draft/sent/viewed/signed/declined/voided, signer_name/email, provider, provider_request_id, signed_path, sent/viewed/signed_at, audit jsonb). RLS: members non-worker read, owner/admin/editor write. Private `contracts` storage bucket (source + signed PDFs, signed-URL access).
...
- **Actions** `src/lib/actions/contracts.ts`: templates + contracts CRUD (draft-only edit), `uploadContractPdf`, `getContractPdfUrl` (signed URL), `renderContractHtml` (merge-field fill), `isSigningEnabled`, `sendContractForSignature`.
- **E-signature = Dropbox Sign, ONE platform account, env-gated by `DROPBOX_SIGN_API_KEY`** (+ optional `DROPBOX_SIGN_TEST_MODE=1`). `sendContractForSignature` posts to `signature_request/send` (PDF as-is; rich-text flattened to an HTML file) and flips to `sent`. Webhook `/api/contracts/dropbox-sign/webhook` (whitelisted in middleware) verifies `event_hash` = HMAC-SHA256(`event_time+event_type`, apiKey), advances status, downloads + stores the final signed PDF. Until the key is set, the UI shows "set up e-signature" and the send button explains it's off.
- **Surfaces:** `/contracts` (list + create modal), `/contracts/[id]` (preview/edit/send/void/delete/download signed). Sidebar "Contracts" under Sales. MCP: `list_contracts`, `get_contract`, `create_contract`, `update_contract`, `void_contract`, `delete_contract`, `list_contract_templates`, `create_contract_template` (scopes `contracts:read`/`contracts:write`).
- **To turn signing on:** set `DROPBOX_SIGN_API_KEY` on Vercel + register the Dropbox Sign account event callback → `https://www.kireihq.com/api/contracts/dropbox-sign/webhook`.
